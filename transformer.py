...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_internal):
        ...
        # considering residual to be added back in, think its supposed to be d_model
        self.value = nn.Linear(d_model, self.d_value) 

        self.fc1 = nn.Linear(self.d_value, self.hidden_dim)
    ...
    def forward(self, input_vecs):
        ...
        """
        #Assume Input is positionally encoded
         
        #Create Query, Key, Value matrices
        querys = self.query(input_vecs) #(seq_len, d_internal)
        keys = self.key(input_vecs) # (seq_len, d_internal)
        ...
        #add residuals
        if self.residual:
            self_attention = self_attention + input_vecs # (seq_len, d_internal)

        #Feed Forward
        fc1 = self.fc1(self_attention)# (seq_len, hidden_dim)
        fc1 = nn.functional.relu(fc1)# (seq_len, hidden_dim)
...
def train_classifier(args, train, dev):
    ...
    for t in range(0, num_epochs):
        ...
        # ex_idxs = [i for i in range(0, len(train_set))]
